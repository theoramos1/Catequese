...
<div class="container" id="contentor">
    ...
    <div class="no-print">
        ...
        <ul class="nav nav-tabs">
            ...
            <li role="presentation" class="active"><a href="analisarCrismas.php">Crismas</a></li>
        </ul>
    </div>
    ...
    try
    {
        ...
        $result = $db->getChrismationAnalysis(Utils::currentCatecheticalYear(), Authenticator::isAdmin(), Authenticator::getUsername());
        ...
        if(!empty($result))
        {
            foreach ($result as $row)
            {
                ...
                {
                    dataCrismaValida($candidato, RELATORIO::AVISO);
                    paroquiaCrismaValida($candidato, RELATORIO::AVISO);
                    array_push($concluidos, $candidato);
                }
                else if (!hasCatecismoMinimoCrisma($candidato, RELATORIO::FATAL) || !hasNumMinimoInscricoes($candidato, RELATORIO::FATAL))
                {
                ...
                else if (hasBaptismo($candidato, RELATORIO::IGNORAR))
                {
                    ...
                    if (!hasPrimeiraComunhao($candidato, RELATORIO::IGNORAR))
                        hasPrimeiraComunhao($candidato, RELATORIO::INFO);        // Registar o aviso
