...
<html lang="pt">
<head>
    <title>Apoio à decisão -- Baptismos</title>
    <meta charset="utf-8">
...
<div class="container" id="contentor">
    ...
    <div class="no-print">
        ...
        <ul class="nav nav-tabs">
            <li role="presentation" class="active"><a href="analisarBaptismos.php">Baptismos</a></li>
            <li role="presentation"><a href="analisarComunhoes.php">Primeiras Comunhões</a></li>
    ...
    try
    {
        ...
        $result = $db->getBaptismAnalysis(Utils::currentCatecheticalYear(), Authenticator::isAdmin(), Authenticator::getUsername());
        ...
        if (!empty($result))
        {
            foreach ($result as $row)
            {
                ...
                if (hasBaptismo($candidato, RELATORIO::IGNORAR))
                {
                    ...
                    if (!hasPrimeiraComunhao($candidato, RELATORIO::IGNORAR))    // O comprovativo so e' relevante para quem for fazer a Primeira Comunhao
                        hasComprovativoBaptismo($candidato, RELATORIO::INFO);
                    array_push($concluidos, $candidato);
                }
                else if (!hasNumMinimoInscricoes($candidato, RELATORIO::FATAL))
                {
