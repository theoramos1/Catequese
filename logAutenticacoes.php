...
<div class="container" id="contentor">
  ...
  <li role="presentation" ><a href="logCatechesis.php">Actividade no CatecheSis</a></li>
  <li role="presentation" class="active"><a href="logAutenticacoes.php">Autenticações</a></li>
  </ul>
  
  
  <div class="row" style="margin-bottom:20px; "></div>
  
  <table class="table table-hover" id="autenticacoes">
  <thead>
	<tr>
		<th>Data / hora</th>
		<th>Utilizador</th>
  ...
  <?php
    ...
    try
    {
      $result = $db->getAuthenticationsLog();
    }
    ...
    if($result && count($result) >= 1)
    {
        foreach($result as $row)
        {
            ...
            echo("\t<td>" . Utils::sanitizeOutput($row['user']) . "</td>\n");
            echo("\t<td>" . $row['action'] . "</td>\n");
            echo("\t<td>" . $row['comment'] . "</td>\n");
            echo("\t<td>" . Utils::sanitizeOutput($row['ip']) . "</td>\n");
